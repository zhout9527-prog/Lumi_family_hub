...
```text
artifacts\windows\Lumi-Server_0.5.1_x64-setup.exe
```

不想安装时，完整保留并运行便携目录；其中两个 EXE 缺一不可：

```text
artifacts\windows\Lumi-Server_0.5.1_x64-portable\
  lumi-server.exe
  lumi-server-core\
...
`artifacts\windows\lumi-server_0.5.1_x64.exe` 可以直接在产物目录中双击，但必须和同目录的 `lumi-server-core\` 文件夹一起保留；移动到其他电脑时应整体复制便携目录。安装版会自动把两部分安装到正确位置。
...
前端组件只维护一份，但使用 Client/Server 两个编译模式生成不同产品。Client 本身不携带数据库或家庭媒体库；Server 安装包会携带并自动启动服务端核心。Windows Client 默认连接本机 `127.0.0.1:8000`，Android Client 会记住首次填写的家庭主机地址。当前 Windows 发布版本为 Server `0.5.1`、Client `0.4.2`。
...
公开内容无需账号。需要账号清晰度或私有收藏夹时，运维人员可在 Server 的“资源管理”中先验证任意一个有效的家长或运维 Lumi 账户，再从本机已登录的 Edge、Chrome 或 Firefox 导入 B站登录状态。平台不接收 B站账号密码，登录凭据不会返回给 Client；断开账号会立即移除导入状态。该能力不绕过会员、付费、DRM、验证码或地区限制，站点登录失效后需要由成人重新导入。
