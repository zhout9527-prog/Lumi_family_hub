...
- Client 内置跨 Windows、Android 手机和 Android TV 的俄罗斯方块、3D 彩块防线、深岩淘金、贪吃蛇与小小画室，规则内核统一，分别适配键盘、触控与电视遥控器
...
```text
artifacts\windows\Lumi-Server_1.4.15_x64-setup.exe
```

不想安装时，完整保留并运行便携目录；其中两个 EXE 缺一不可：

```text
artifacts\windows\Lumi-Server_1.4.15_x64-portable\
  lumi-server.exe
  lumi-server-core\
...
`artifacts\windows\lumi-server_1.4.15_x64.exe` 可以直接在产物目录中双击，但必须和同目录的 `lumi-server-core\` 文件夹一起保留；移动到其他电脑时应整体复制便携目录。安装版会自动把两部分安装到正确位置。
...
前端组件只维护一份，但使用 Client/Server 两个编译模式生成不同产品。Client 本身不携带数据库或家庭媒体库；Server 安装包会携带并自动启动服务端核心。Windows Client 默认连接本机 `127.0.0.1:2521`，Android Client 会记住首次填写的家庭主机地址。当前 Windows、Android 发布版本统一为 `1.4.15`。
...
2. “在线播放”可登记 B站视频、抖音/夸克官方分享页以及 HTTPS 视频、音频、PDF 等开放直链。B站视频由 Server 按需解析并临时封装成带短时票据的 HLS 流，Client 继续使用 Lumi 播放器，同时加载可用的弹幕和评论；夸克分享页若明确公开 HTTPS 视频预览，Lumi 会在每次播放时重新获取临时地址并用内置播放器打开。需要登录、提取码、转存或受 DRM/站点策略限制的分享会保留“在官方页面打开”入口。
...
“小游戏”当前包含俄罗斯方块、3D 彩块防线、深岩淘金、完整版贪吃蛇、小小画室和画线人官方游戏入口，儿童与家长 Client 均可进入，Server 运维界面不加载游戏入口。俄罗斯方块在 Windows 使用方向键移动和旋转、空格直接落下，手机使用触控和滑动，电视由遥控器直接控制。彩块防线使用 Three.js 呈现高密度立体方阵，发射器自动攻击最近的匹配方块，并提供换装充能、备用池、保底补给和局外成长；三档推进速度较上一版降低 20%，起点推远约 20%，场景延长约 20%，只能挑战下一关或重玩已经通关的关卡。小小画室包含 9 幅原有的简单和中等课程，以及从 Lucide 官方 ISC 开源线稿中生成的 117 幅儿童临摹图；支持按主题搜索、30 条每日创作灵感、喷漆、毛笔、水墨、平刷、文字、任意颜色和 50% 至 300% 精细缩放。手机或鼠标切换“拖动画布”后可向四周移动，接近边缘时会按块扩展新的可绘制区域，完整扩展画布会随作品一起导出。开源图案出处和完整许可见 `THIRD_PARTY_NOTICES.md` 与 `public/licenses/lucide-isc.txt`。贪吃蛇在手机使用滑动或触控方向键，在 Windows 使用方向键或 WASD，电视使用方向键转向、确认键暂停或继续、返回键退出。画线人为保留原版美术和关卡，通过 Lumi 原生内嵌浏览窗口访问官方网站，不复制其代码或素材。

深岩淘金采用十个主题关卡并可继续进入无尽矿层：前三关提供无遮挡教学目标和瞄准线，之后逐步提高累计积分目标。积分同时是过关进度和商店货币，购买道具后把余额带入下一关，不再额外发放独立矿币。金块、钻石和福袋采用更有重量感的慢速上拉；高价值物品清空且达到目标后自动结算。第 15 关起出现会炸毁周围物品的炸药桶，第 20 关起加入废篮球、废瓶子和旧轮胎等低价值干扰物。背包道具可以跨关积攒，电视和 Windows 均可按向上键快捷使用炸药。游戏进度、背包和最高分只保存在当前 Client 设备，不上传家庭主机，也不参与儿童排名。
