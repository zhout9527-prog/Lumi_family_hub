...
下载完成但尚未审核的文件不会丢失，也不会直接对 Client 可见。默认会先保存在 `runtime\library\video\.pending`（可在“主机设置”里改成其他待审核目录），数据库状态为“待审核”；审核通过后复制到 `runtime\library\video`，拒绝或冻结的文件仍会留在待审核目录，方便运维复核或清理。旧版本 `runtime\quarantine` 中的文件会在 Server 启动时自动迁移到这个目录；即使文件暂时被占用而无法迁移，审核接口仍能读取旧目录中的文件。
