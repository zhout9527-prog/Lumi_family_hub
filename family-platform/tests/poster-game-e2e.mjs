...
try {
  ...
  check(await child.getByRole('tab', { name: /简单/ }).count() === 1 && await child.getByRole('tab', { name: /中等/ }).count() === 1 && await child.getByRole('tab', { name: /复杂/ }).count() === 1, '小小画室没有简单、中等、复杂三级临摹')
  check(await child.locator('.art-template-picker button').count() === 6, '简单临摹课题不是 6 个')
  await child.getByRole('tab', { name: /中等/ }).click()
  check(await child.locator('.art-template-picker button').count() === 3, '中等临摹课题不是 3 个')
  await child.getByRole('tab', { name: /复杂/ }).click()
  check(await child.locator('.art-template-picker button').count() === 3, '复杂临摹课题不是 3 个')
  check(await child.locator('.art-reference-heading').getAttribute('data-art-difficulty') === 'advanced', '复杂课题没有切换成功')
  check(await child.locator('.art-lesson-card li').count() === 3, '复杂课题没有分步骤美术指导')
