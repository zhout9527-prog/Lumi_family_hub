...
```text
artifacts\windows\Lumi-Server_1.4.9_x64-setup.exe
```

不想安装时，完整保留并运行便携目录；其中两个 EXE 缺一不可：

```text
artifacts\windows\Lumi-Server_1.4.9_x64-portable\
  lumi-server.exe
  lumi-server-core\
...
`artifacts\windows\lumi-server_1.4.9_x64.exe` 可以直接在产物目录中双击，但必须和同目录的 `lumi-server-core\` 文件夹一起保留；移动到其他电脑时应整体复制便携目录。安装版会自动把两部分安装到正确位置。
...
前端组件只维护一份，但使用 Client/Server 两个编译模式生成不同产品。Client 本身不携带数据库或家庭媒体库；Server 安装包会携带并自动启动服务端核心。Windows Client 默认连接本机 `127.0.0.1:2521`，Android Client 会记住首次填写的家庭主机地址。当前 Windows、Android 发布版本统一为 `1.4.9`。
...
“小游戏”当前内置俄罗斯方块、方块割草和深岩淘金，儿童与家长 Client 均可进入，Server 运维界面不加载游戏入口。俄罗斯方块在 Windows 使用方向键移动和旋转、空格直接落下，手机使用触控和滑动，电视由遥控器直接控制。方块割草在三端均按格移动与消除。深岩淘金采用十个主题关卡：前三关提供无遮挡教学目标和瞄准线，后续分段提高目标；大金块、小金块、钻石和石块具有明显不同的回收速度。每关结束进入补给商店，福袋保证给分数与矿币并可能额外掉落福袋徽章、大力手套、炸药或两类幸运书；道具可以积攒并由玩家决定何时使用。最后一钩照常结算，接近目标时会有一次友好补时，达到目标后也可提前收工。游戏进度、背包和最高分只保存在当前 Client 设备，不上传家庭主机，也不参与儿童排名。
