...
- Client 内置跨 Windows、Android 手机和 Android TV 的俄罗斯方块、方块割草和深岩淘金，规则内核统一，分别适配键盘、触控与电视遥控器
...
```text
artifacts\windows\Lumi-Server_1.4.10_x64-setup.exe
```

不想安装时，完整保留并运行便携目录；其中两个 EXE 缺一不可：

```text
artifacts\windows\Lumi-Server_1.4.10_x64-portable\
  lumi-server.exe
  lumi-server-core\
...
`artifacts\windows\lumi-server_1.4.10_x64.exe` 可以直接在产物目录中双击，但必须和同目录的 `lumi-server-core\` 文件夹一起保留；移动到其他电脑时应整体复制便携目录。安装版会自动把两部分安装到正确位置。
...
前端组件只维护一份，但使用 Client/Server 两个编译模式生成不同产品。Client 本身不携带数据库或家庭媒体库；Server 安装包会携带并自动启动服务端核心。Windows Client 默认连接本机 `127.0.0.1:2521`，Android Client 会记住首次填写的家庭主机地址。当前 Windows、Android 发布版本统一为 `1.4.10`。
...
“小游戏”当前包含俄罗斯方块、方块割草、深岩淘金、完整版贪吃蛇和画线人官方游戏入口，儿童与家长 Client 均可进入，Server 运维界面不加载游戏入口。俄罗斯方块在 Windows 使用方向键移动和旋转、空格直接落下，手机使用触控和滑动，电视由遥控器直接控制。方块割草在三端均按格移动与消除。贪吃蛇在手机使用滑动或触控方向键，在 Windows 使用方向键或 WASD，电视使用方向键转向、确认键暂停或继续、返回键退出。画线人为保留原版美术和关卡，只在手机和 Windows 显示官方网页入口，不在 Lumi 内复制其代码或素材。

深岩淘金采用十个主题关卡：前三关提供无遮挡教学目标和瞄准线，后续分段提高目标；大金块、小金块、钻石和石块具有明显不同的回收速度，抓到目标后的上拉速度比放钩速度更有重量感。每关结束进入补给商店，福袋保证给分数与矿币并可能额外掉落福袋徽章、大力药水、炸药或两类幸运书；背包道具可以跨关积攒，电视和 Windows 均可按向上键快捷使用炸药。最后一钩照常结算，接近目标时会有一次友好补时，达到目标后也可提前收工。游戏进度、背包和最高分只保存在当前 Client 设备，不上传家庭主机，也不参与儿童排名。
