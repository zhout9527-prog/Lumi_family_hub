...
try {
  ...
  await page.getByRole('button', { name: '切换合集选集' }).click({ force: true })
  check(await page.locator('.video-episode-menu button').count() === 2, '全屏播放器没有显示完整选集')
  await page.locator('.video-episode-menu button').filter({ hasText: '认识天空' }).click()
  await page.locator('.video-title').filter({ hasText: '认识天空' }).waitFor()
  check(Boolean(await page.evaluate(() => document.fullscreenElement)), '全屏切换分集后退出了全屏')
  await page.getByRole('button', { name: '切换合集选集' }).click({ force: true })
  const autoNext = page.getByLabel('自动连播下一集')
  await autoNext.check()
  check(await autoNext.isChecked(), '自动连播下一集没有启用')
  await page.getByRole('button', { name: '切换合集选集' }).click({ force: true })
  await page.locator('.lumi-video-player video').dispatchEvent('ended')
  await page.locator('.video-title').filter({ hasText: '认识海洋' }).waitFor()
  check(Boolean(await page.evaluate(() => document.fullscreenElement)), '自动连播下一集后退出了全屏')
