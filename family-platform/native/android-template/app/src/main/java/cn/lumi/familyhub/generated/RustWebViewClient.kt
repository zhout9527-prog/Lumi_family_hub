...
class RustWebViewClient(webView: RustWebView, context: Context): WebViewClient() {
    ...
    override fun shouldInterceptRequest(
        ...
        return if (Rust.withAssetLoader((view as RustWebView).id)) {
            ...
            if (response != null) {
                val mimeType = response.mimeType?.lowercase().orEmpty()
                if (
                    response.encoding.isNullOrBlank() &&
                    (mimeType.startsWith("text/") ||
                        mimeType == "application/javascript" ||
                        mimeType == "application/json")
                ) {
                    // Some Android WebViews ignore the HTML meta tag when the
                    // intercepted response itself does not declare an encoding.
